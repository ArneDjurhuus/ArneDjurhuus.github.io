...
export default async function SpaceTodoPage({ params }: { params: { slug: string } }) {
  let space: any = null;
  ...
  // Compute a clean dashboard href depending on host (subdomain vs path-based)
  const hdrs = headers();
  ...
  if (parts.length === 2 && parts[1] === 'localhost') {
    hasSubdomain = true; // demo.localhost
  } else if (parts.length >= 3) {
    hasSubdomain = true; // foo.squadspace.me
  }
  if (parts[0] === 'www') {
    hasSubdomain = false;
  }
  ...
  const dashboardHref = hasSubdomain ? '/' : `/spaces/${params.slug}`;

  if (error) {
    return (
      <main style={{ minHeight: '100vh', padding: 32, background: '#0b1220', color: 'white' }}>
        <Link href={dashboardHref} style={{ color: '#4fd1c5' }}>&larr; Dashboard</Link>
  ...
  return (
    <main style={{ minHeight: '100vh', padding: 32, background: '#0b1220', color: 'white' }}>
      <Link href={dashboardHref} style={{ color: '#4fd1c5' }}>&larr; Dashboard</Link>
      <h1 style={{ fontSize: 32, fontWeight: 800, marginTop: 16 }}>{space.name} · To‑Do</h1>
      ...
      <form action={async (formData: FormData) => {
        ...
        revalidatePath(`/spaces/${space.slug}/todo`);
      }} style={{ marginTop: 16, display: 'grid', gap: 8, gridTemplateColumns: '1fr auto auto' }}>
        <input name="title" placeholder="Title" style={{ padding: 10, borderRadius: 8, border: '1px solid #1f2a44', background: '#0f172a', color: 'white' }} />
        ...
        <textarea name="description" placeholder="What is this task about?" rows={2} style={{ gridColumn: '1 / -1', width: '100%', padding: 10, borderRadius: 8, border: '1px solid #1f2a44', background: '#0f172a', color: 'white' }} />
      </form>
                  ...
                  <div style={{ background: '#0f172a', border: '1px solid #1f2a44', borderRadius: 10, padding: 12 }}>
                    ...
                    <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      {[
                        ...
                        .filter((opt: any) => opt.key !== t.status)
                        .map((opt: any) => (
                          <button
                            key={opt.key}
                            name="target"
      ...
      <form action={async () => {
        'use server';
        // Log a digest request event and refresh
